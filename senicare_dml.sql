...
-- 필요한 추가 데이터 X
-- 추가 조치 : 아디로만 조회를 하고 암호화된 비밀번호 체크를 서버측에서 실행


-- SELECT * FROM nurse WHERE id = :id AND password = :password;
SELECT * FROM nurse WHERE id = :id; # 비밀번호는 암호화되어 서버측에서만 실행 가능
...
-- 레코드 삽입시 전화번호 중복 검증
SELECT * FROM nurse WHERE tel_auth = '1234';
SELECT * FROM tel_auth WHERE tel_number = '01012345678' AND = '01012345678'; # 인증번호 받은것과 실제 인증번호가 맞는지 확인하기 위해 AND를 사용함
...
-- 2. 이미 인증 절치가 진행중인 전화번호가 있다면 해당 전화번호에 대한 인증 번호를 수정하는 작업으로 대체
...
SELECT * FROM tel_auth WHERE tel_number = '01012345678' AND auth_number = '0123'; # 인증번호 받은것과 실제 인증번호가 맞는지 확인하기 위해 AND를 사용함
...
VALUES('이영희','부산광역시','qwer1234', null, '59086', '부산광역시 부산진구...');
...
-- 고객 상세보기
-- 최종 사용자 (고객 정보) > (고객 번호, 고객 사진, 고객 이름, 생년 월일 ,담당자 이름, 주소)
...
-- 관리 기록 테이블에서 특정 고객에 댛나 관리 기록 리스트를 날짜 순으로 내림차순 정렬하여 조회
-- 필요 추가 데이터 : 고객 번호(화면에서)
SELECT management_date, comment, name, used_goods_count
...
-- 필요 추가 데이터 : 고객번호(화면에서), 날짜(서버에서 생성)

-- 레코드 삽입시 고객 관계 검증
SELECT * FROM customer WHERE customer_number = 1;
-- 레코드 삽입시 용품 관계 검증(사용하는 용품이 있을 경우)
SELECT * FROM goods WHERE goods_number = 1;
#존재하지 않는다면 내보내고, 존재한다면 goods를 검사하여 삽입하는 작업을 한다.
INSERT INTO customer_management VALUES (1,'2024-07-26', '최초방문', null, null);
...
UPDATE customer SET profile_image = 'http:///~~~',
name = '고길동',
birth = '590830',
charger = 1,
address = '부산광역시 부산진구...'
